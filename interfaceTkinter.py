...
def telaEstudante(student):
        ...
        buttonForm['command'] = lambda: telaResponderForm(student, telaEstudante, [ip1, ip1])
        buttonForm.pack()
...
def telaResponderForm(student, telaEstudante, listaIPs):
    
    ### Nova tela ###
    telaResponderForm1 = Toplevel(telaEstudante)
    telaResponderForm1.title(f'Formulário de avaliação de {student}')
    
    ### Label formulário ###
    labelForm = Label(telaResponderForm1)
    labelForm['text'] = listaIPs[0].enunciado
    labelForm['font'] = ('Arial', '13', 'bold')
    labelForm.pack()

    ### Frame de checkboxes ###
    frameCheckboxes = Frame(telaResponderForm1)
    frameCheckboxes['pady'] = 10
    frameCheckboxes.pack()

    for i in list(listaIPs[0].respostas.keys()):
        checkbox = Checkbutton(frameCheckboxes)
        checkbox['text'] = listaIPs[0].respostas[i]
        checkbox.pack()
    
    buttonProximo = Button(frameCheckboxes)
    buttonProximo['text'] = "Próximo"
    buttonProximo['command'] = lambda : telaResponderForm(student, telaEstudante, listaIPs[1:]) if len(listaIPs) > 1 else telaResponderForm1.destroy()
    buttonProximo.pack()
